...
- 新增本地 `漫画阅读器`：
  - 扫描漫画目录
  - 查看图片文件夹、`CBZ`、`ZIP`、`PDF`、`7z`
  - 在系统已安装 `unrar` / `7z` / `unar` / `bsdtar` 时，可额外打开 `CBR` / `RAR`
  - 图片文件夹及压缩包内支持 `jpg` / `jpeg` / `png` / `webp` / `gif` / `bmp`
  - 显示页数、大小、修改时间、路径
  - 直接在 GUI 中翻页阅读
  - 支持 `适应窗口`、`适应宽度` 和 `自定义缩放`
  - 记住上次目录、选中的漫画和阅读页码
  - 打开原文件和所在目录
- 支持 `aria2c` 加速下载。
- 支持 Playwright/Chromium 依赖自动安装。
- 支持将图片文件夹、`CBZ`、`ZIP`、`PDF`、`7z` 转换为新的 `CBZ` 文件。
- 若系统里存在 `unrar`、`7z`、`unar` 或 `bsdtar`，也支持将 `CBR`、`RAR` 转换为 `CBZ`。
...
1. 打开 `漫画阅读器` 页面。
2. 选择漫画根目录，或者直接选择单个 `CBZ` / `ZIP` / `PDF` / `7z` 文件。
   如果系统里有 `unrar` / `7z` / `unar` / `bsdtar`，也可以直接选择 `CBR` / `RAR`。
3. 点击 `刷新列表` 扫描本地漫画。
4. 在左侧列表选择条目后，可以先查看文件信息。
5. 点击 `开始阅读` 后可使用 `首页`、`上一页`、`下一页`、`末页` 和页码跳转阅读。
6. 阅读区支持 `-`、`100%`、`+` 缩放，以及 `适应窗口`、`适应宽度`、`自定义缩放` 三种模式。
7. 支持 `Left` / `Right`、`PageUp` / `PageDown`、`Home` / `End` 快捷翻页。
8. `打开文件` 可用系统默认程序查看原文件，`打开所在目录` 可直接跳到本地位置。
9. 关闭后重新打开 GUI，会尽量恢复到上次阅读的位置和缩放设置。

## 转换为 CBZ 使用说明

1. 打开 `转换为 CBZ` 页面。
2. 输入图片文件夹、`CBZ`、`ZIP`、`PDF`、`CB7`、`7z`、`CBR`、`RAR` 之一作为输入。
3. 选择输出的 `.cbz` 文件路径。
4. 点击 `开始转换`。
5. 如果输入是 `CBR` / `RAR`，而系统里没有 `unrar`、`7z`、`unar`、`bsdtar`，GUI 会先给出提示。
...
- 本地阅读器当前支持：图片文件夹、`CBZ`、`ZIP`、`PDF`、`7z`。
- 若系统里存在 `unrar`、`7z`、`unar` 或 `bsdtar`，阅读器也可以直接打开 `CBR`、`RAR`。
- 阅读器中的图片页格式支持：`jpg`、`jpeg`、`png`、`webp`、`gif`、`bmp`。
- “系列下载”流程里会识别已下载的 `CBZ` / `CBR` 文件名用于后续重命名；其中 `CBR` 在阅读器里仍依赖系统可用的解包工具。
- “转换为 CBZ” 当前支持图片文件夹、`CBZ`、`ZIP`、`PDF`、`CB7`、`7z`。
- 若系统里存在 `unrar`、`7z`、`unar` 或 `bsdtar`，“转换为 CBZ” 也支持 `CBR`、`RAR`。
